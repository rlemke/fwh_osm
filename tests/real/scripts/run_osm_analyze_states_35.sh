#!/usr/bin/env bash
# Analyze 35 US States — convenience startup script
#
# Runs 10 composed analysis workflows (bicycle routes, parks, cities,
# transport, boundaries, etc.) for 35 US states.
#
...
#
# Usage:
#   examples/osm-geocoder/tests/real/scripts/run_osm_analyze_states_35.sh
#
set -euo pipefail
...
# ---------------------------------------------------------------------------
echo "=== Compiling osm_analyze_states_35.afl ==="
AFL_FILE="$REAL_DIR/ffl/osm_analyze_states_35.ffl"
OUTPUT_FILE="$REAL_DIR/osm_analyze_states_35.json"
...
# ---------------------------------------------------------------------------
echo "=== Submitting AnalyzeStates_35 workflow ==="
export AFL_MONGODB_URL="mongodb://afl-mongodb:27017"
"$PYTHON" -m afl.runtime.submit \
    --primary "$AFL_FILE" \
    "${LIB_ARGS[@]}" \
    --workflow "osm.UnitedStates.analysis.AnalyzeStates_35"
echo ""
