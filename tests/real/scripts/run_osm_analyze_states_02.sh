#!/usr/bin/env bash
# Analyze 2 US States — convenience startup script
#
# Runs 10 composed analysis workflows (bicycle routes, parks, cities,
# transport, boundaries, etc.) for 2 US states.
#
...
# ---------------------------------------------------------------------------
echo "=== Compiling osm_analyze_states_02.afl ==="
AFL_FILE="$REAL_DIR/ffl/osm_analyze_states_02.ffl"
OUTPUT_FILE="$REAL_DIR/osm_analyze_states_02.json"
...
# ---------------------------------------------------------------------------
echo "=== Submitting AnalyzeStates_02 workflow ==="
export AFL_MONGODB_URL="mongodb://afl-mongodb:27017"
"$PYTHON" -m afl.runtime.submit \
    --primary "$AFL_FILE" \
    "${LIB_ARGS[@]}" \
    --workflow "osm.UnitedStates.analysis.AnalyzeStates_02"
echo ""
