#!/usr/bin/env bash
# Download 45 US States — convenience startup script
#
...
#
# Usage:
#   examples/osm-geocoder/tests/real/scripts/run_osm_cache_states_45.sh
#
set -euo pipefail
...
# ---------------------------------------------------------------------------
echo "=== Compiling osm_cache_states_45.afl ==="
AFL_FILE="$REAL_DIR/ffl/osm_cache_states_45.ffl"
OUTPUT_FILE="$REAL_DIR/osm_cache_states_45.json"
...
# ---------------------------------------------------------------------------
echo "=== Submitting DownloadStates_45 workflow ==="
...
python -m afl.runtime.submit \
    ...
    --library "$EXAMPLE_DIR/handlers/downloads/ffl/osmoperations.ffl" \
    --library "$EXAMPLE_DIR/handlers/cache/ffl/osmcache.ffl" \
    --workflow "osm.UnitedStates.cache.DownloadStates_45"
echo ""
