#!/usr/bin/env bash
# Download 30 US States — convenience startup script
#
...
#
# Usage:
#   examples/osm-geocoder/tests/real/scripts/run_osm_cache_states_30.sh
#
set -euo pipefail
...
# ---------------------------------------------------------------------------
echo "=== Compiling osm_cache_states_30.afl ==="
AFL_FILE="$REAL_DIR/ffl/osm_cache_states_30.ffl"
OUTPUT_FILE="$REAL_DIR/osm_cache_states_30.json"
...
# ---------------------------------------------------------------------------
echo "=== Submitting DownloadStates_30 workflow ==="
...
python -m afl.runtime.submit \
    ...
    --library "$EXAMPLE_DIR/handlers/downloads/ffl/osmoperations.ffl" \
    --library "$EXAMPLE_DIR/handlers/cache/ffl/osmcache.ffl" \
    --workflow "osm.UnitedStates.cache.DownloadStates_30"
echo ""
