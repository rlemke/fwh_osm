#!/usr/bin/env bash
# Analyze All US States — convenience startup script
#
# Runs 10 composed analysis workflows (bicycle routes, parks, cities,
# transport, boundaries, etc.) for all 50 US states plus DC.
#
...
#
# Usage:
#   examples/osm-geocoder/tests/real/scripts/run_osm_analyze_states.sh
#
set -euo pipefail
...
# ---------------------------------------------------------------------------
echo "=== Compiling osm_analyze_states.afl ==="
AFL_FILE="$REAL_DIR/ffl/osm_analyze_states.ffl"
OUTPUT_FILE="$REAL_DIR/osm_analyze_states.json"
...
"$PYTHON" -m afl.runtime.submit \
    --primary "$AFL_FILE" \
    "${LIB_ARGS[@]}" \
    --workflow "osm.UnitedStates.analysis.AnalyzeAllStates"
echo ""
