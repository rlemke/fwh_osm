#!/usr/bin/env bash
# Download 40 US States — convenience startup script
#
...
#
# Usage:
#   examples/osm-geocoder/tests/real/scripts/run_osm_cache_states_40.sh
#
set -euo pipefail
...
# ---------------------------------------------------------------------------
echo "=== Compiling osm_cache_states_40.afl ==="
AFL_FILE="$REAL_DIR/ffl/osm_cache_states_40.ffl"
OUTPUT_FILE="$REAL_DIR/osm_cache_states_40.json"
...
# ---------------------------------------------------------------------------
echo "=== Submitting DownloadStates_40 workflow ==="
...
python -m afl.runtime.submit \
    ...
    --library "$EXAMPLE_DIR/handlers/downloads/ffl/osmoperations.ffl" \
    --library "$EXAMPLE_DIR/handlers/cache/ffl/osmcache.ffl" \
    --workflow "osm.UnitedStates.cache.DownloadStates_40"
echo ""
