#!/usr/bin/env bash
# Analyze 1 US State — convenience startup script
#
# Runs 10 composed analysis workflows (bicycle routes, parks, cities,
# transport, boundaries, etc.) for 1 US state.
#
...
#
# Usage:
#   examples/osm-geocoder/tests/real/scripts/run_osm_analyze_states_01.sh
#
set -euo pipefail
...
# ---------------------------------------------------------------------------
echo "=== Compiling osm_analyze_states_01.afl ==="
AFL_FILE="$REAL_DIR/ffl/osm_analyze_states_01.ffl"
OUTPUT_FILE="$REAL_DIR/osm_analyze_states_01.json"
...
"$PYTHON" -m afl.runtime.submit \
    --primary "$AFL_FILE" \
    "${LIB_ARGS[@]}" \
    --workflow "osm.UnitedStates.analysis.AnalyzeStates_01"
echo ""
