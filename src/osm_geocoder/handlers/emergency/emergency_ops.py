...
_ABOUT = (
    "<b>Every number in a city popup, and how it is calculated:</b>"
    "<dl style='margin:.5em 0'>"
    "<dt><b>score</b></dt><dd>The city's readiness, 0&ndash;100: the "
    "equal-weight mean of the per-category <i>comp</i> values (a category "
    "with no mapped facilities region-wide is dropped and the weights "
    "renormalize). <b>Equal weights are a disclosed value judgment.</b></dd>"
    "<dt><b>pop</b></dt><dd>The OSM <code>population</code> tag on the "
    "city's place node; also the weight used in the region score.</dd>"
    "<dt><b>nearest</b></dt><dd>Routed <b>network distance</b> (km) from "
    "the city's place node to the closest facility of the category: the 5 "
    "closest facilities by straight line are each routed over the region's "
    "major-road (motorway&ndash;secondary) network and the smallest routed "
    "distance is kept. Approximate corridor-level routing, not "
    "door-to-door.</dd>"
    "<dt><b>count</b></dt><dd>All facilities of the category mapped in the "
    "ENTIRE region's OSM extract &mdash; coverage context only, it feeds no "
    "score. A low count can mean under-mapped, not absent.</dd>"
    "<dt><b>comp</b></dt><dd>The category's score contribution: "
    "<code>100 &times; (1 &minus; nearest / 50)</code> clamped to "
    "0&ndash;100 &mdash; 0 km &rarr; 100, 25 km &rarr; 50, &ge;50 km or no "
    "facilities &rarr; 0.</dd>"
    "<dt><b>region score</b> (rankings)</dt><dd>The population-weighted "
    "mean of the region's city scores (unweighted when no city has a "
    "population tag).</dd>"
    "</dl>"
    "Facilities come from each region's OSM extract (never live APIs); OSM "
    "coverage varies and <b>shelters especially are thinly mapped</b>. "
    "Cities are OSM places tagged <code>place=city|town</code> with a "
    "population tag; admin place nodes are excluded and duplicate nodes "
    "within 5 km dedupe to the most populous. Routing is approximate (see "
