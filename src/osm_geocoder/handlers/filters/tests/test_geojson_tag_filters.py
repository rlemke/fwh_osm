...
def test_contains_case_insensitive_by_default(tmp_path):
    src = _write(tmp_path / "in.geojson",
                 {"name": "Starbucks Coffee"}, {"name": "Blue Bottle"},
                 {"name": "STARBUCKS Reserve"})
    res = filter_geojson_by_tag_contains(src, "name", "starbucks",
                                         output_path=str(tmp_path / "o.geojson"))
...
def test_contains_case_sensitive(tmp_path):
    src = _write(tmp_path / "in.geojson",
                 {"name": "Starbucks Coffee"}, {"name": "STARBUCKS Reserve"})
    res = filter_geojson_by_tag_contains(src, "name", "Starbucks", case_sensitive=True,
                                         output_path=str(tmp_path / "o.geojson"))
...
def test_regex_alternation(tmp_path):
    src = _write(tmp_path / "in.geojson",
                 {"cuisine": "pizza"}, {"cuisine": "italian_pizza"}, {"cuisine": "sushi"})
    res = filter_geojson_by_tag_regex(src, "cuisine", "pizza|italian",
                                      output_path=str(tmp_path / "o.geojson"))
    assert res.feature_count == 2
...
def test_regex_anchored(tmp_path):
    src = _write(tmp_path / "in.geojson",
                 {"ref": "I 5"}, {"ref": "US 101"}, {"ref": "I 80"})
    res = filter_geojson_by_tag_regex(src, "ref", r"^I \d+$",
                                      output_path=str(tmp_path / "o.geojson"))
...
def test_filters_skip_non_string_and_missing(tmp_path):
    src = _write(tmp_path / "in.geojson",
                 {"name": "Cafe"}, {"other": "x"}, {"name": 42})
    res = filter_geojson_by_tag_contains(src, "name", "cafe",
                                         output_path=str(tmp_path / "o.geojson"))
